...
class OvercookedGridworld(object):
    ...
    def __init__(self, terrain, start_player_positions, start_order_list=None, cook_time=20, num_items_for_soup=3, delivery_reward=20, rew_shaping_params=None, layout_name="unnamed_layout"):
        ...
        self.archetype_one = Archetype()
        self.archetype_two = Archetype()
        ...
        for player, action in zip(new_state.players, joint_action):
            ...
            if terrain_type == 'X':
                if player.has_object() and not new_state.has_object(i_pos):
                    ...
                    #Archetype: self.num_of_objects_placed++
                    if iterations == 1:
                        self.archetype_one.num_of_objects_placed = self.archetype_one.num_of_objects_placed + 1
                    if iterations == 2:
                        self.archetype_two.num_of_objects_placed = self.archetype_two.num_of_objects_placed + 1
                ...
                if player.get_object().name == 'dish' and new_state.has_object(i_pos):
                    ...
                    if num_items == self.num_items_for_soup and cook_time >= self.soup_cooking_time:
                        ...
                        #Archetype: self.num_of_soup_plated++
                        if iterations == 1:
                            self.archetype_one.num_of_soup_plated = self.archetype_one.num_of_soup_plated + 1
                        if iterations == 2:
                            self.archetype_two.num_of_soup_plated = self.archetype_two.num_of_soup_plated + 1
                    ...
                    if not new_state.has_object(i_pos):
                        ...
                        #Archetype: self.num_of_objects_boiled++
                        if iterations == 1:
                            self.archetype_one.num_of_objects_boiled = self.archetype_one.num_of_objects_boiled + 1
                        if iterations == 2:
                            self.archetype_two.num_of_objects_boiled = self.archetype_two.num_of_objects_boiled + 1
                ...
                if obj.name == 'soup':
                    ...
                    #Archetype: self.num_of_soup_delivered++
                    if iterations == 1:     
                        self.archetype_one.num_of_soup_delivered = self.archetype_one.num_of_soup_delivered + 1  
                    if iterations == 2:
                        self.archetype_two.num_of_soup_delivered = self.archetype_two.num_of_soup_delivered + 1    
